...
#                                             Constants
# ==================================================================================================
VALIDATOR_KEY_PROPERTY_W_QUOTES='"root_validator_key":'
NETWORK_ID_PROPERTY="network_name"
MASTER_ACCOUNT_PROPERTY="account_id"
NODE_RPC_URL_PROPERTY="near_node_rpc_url"
...
package_exec_dirpath="${NEAR_KURTOSIS_DIRPATH}/${now_str}"
if ! mkdir -p "${package_exec_dirpath}"; then
    echo "Error: Couldn't create directory '${package_exec_dirpath}' to store the package exec output" >&2
    exit 1
fi

exec_output_filepath="${package_exec_dirpath}/exec-output.log"
...
function get_json_property() {
    package_output_filepath="${1:-}"
    property_name="${2:-}"
    if [ -z "${package_output_filepath}" ]; then
        echo "Error: The filepath to the package output must be provided" >&2
        return 1
    fi
...
if ! cat "${exec_output_filepath}" | awk "/${VALIDATOR_KEY_PROPERTY_W_QUOTES}/,/\}/" | sed "s/${VALIDATOR_KEY_PROPERTY_W_QUOTES}//" | sed 's/},/}/' >"${validator_key_filepath}"; then
    echo "Error: Couldn't extract the validator key JSON from package exec logfile '${exec_output_filepath}'" >&2
    exit 1
fi
if ! contract_helper_url="$(get_json_property "${exec_output_filepath}" "${HELPER_URL_PROPERTY}")"; then
    echo "Error: Couldn't extract the contract helper URL from package exec logfile '${exec_output_filepath}'" >&2
    exit 1
fi
if ! network_id="$(get_json_property "${exec_output_filepath}" "${NETWORK_ID_PROPERTY}")"; then
    echo "Error: Couldn't extract the network ID from package exec logfile '${exec_output_filepath}'" >&2
    exit 1
fi
if ! master_account="$(get_json_property "${exec_output_filepath}" "${MASTER_ACCOUNT_PROPERTY}")"; then
    echo "Error: Couldn't extract the master account from package exec logfile '${exec_output_filepath}'" >&2
    exit 1
fi
if ! node_url="$(get_json_property "${exec_output_filepath}" "${NODE_RPC_URL_PROPERTY}")"; then
    echo "Error: Couldn't extract the NEAR node RPC URL from package exec logfile '${exec_output_filepath}'" >&2
    exit 1
fi
if ! helper_url="$(get_json_property "${exec_output_filepath}" "${HELPER_URL_PROPERTY}")"; then
    echo "Error: Couldn't extract the contract helper service URL from package exec logfile '${exec_output_filepath}'" >&2
    exit 1
fi
if ! explorer_url="$(get_json_property "${exec_output_filepath}" "${EXPLORER_URL_PROPERTY}")"; then
    echo "Error: Couldn't extract the explorer URL from package exec logfile '${exec_output_filepath}'" >&2
    exit 1
fi
if ! wallet_url="$(get_json_property "${exec_output_filepath}" "${WALLET_URL_PROPERTY}")"; then
    echo "Error: Couldn't extract the wallet URL from package exec logfile '${exec_output_filepath}'" >&2
    exit 1
fi
